...
resource "datadog_monitor" "cpu_utilization" {
  ...
  query = <<END
    avg(${var.cpu_utilization_evaluation_window}):
      avg:aws.elasticache.cpuutilization${local.query_filter} by {cachenodeid,cacheclusterid,region,aws_account,env}
...
resource "datadog_monitor" "cpu_utilization_anomaly" {
  ...
  query = <<END
    avg(${var.cpu_utilization_evaluation_window}):anomalies(
      avg:aws.elasticache.cpuutilization${local.query_filter} by {cachenodeid,cacheclusterid,region,aws_account,env}, 'agile', ${var.cpu_utilization_anomaly_deviations},
...
resource "datadog_monitor" "evictions" {
  ...
  query = <<END
    sum(${var.evictions_evaluation_window}): (
      avg:aws.elasticache.evictions${local.query_filter} by {cachenodeid,cacheclusterid,region,aws_account,env}
...
resource "datadog_monitor" "hit_rate" {
  ...
  query = <<END
    max(${var.hit_rate_evaluation_window}):(
      avg:aws.elasticache.cache_hit_rate${local.query_filter} by {cachenodeid,cacheclusterid,region,aws_account,env}
...
resource "datadog_monitor" "hit_rate_anomaly" {
  ...
  query = <<END
    avg(${var.hit_rate_evaluation_window}):anomalies(
      avg:aws.elasticache.cache_hit_rate${local.query_filter} by {cachenodeid,cacheclusterid,region,aws_account,env}, 'agile', ${var.hit_rate_anomaly_deviations},
...
resource "datadog_monitor" "max_connections" {
  ...
  query = <<END
    max(${var.max_connections_evaluation_window}):(
      avg:aws.elasticache.curr_connections${local.query_filter} by {cachenodeid,cacheclusterid,region,aws_account,env}
...
resource "datadog_monitor" "swap_usage" {
  ...
  query = <<END
    max(${var.swap_usage_evaluation_window}):(
      avg:aws.elasticache.swap_usage${local.query_filter} by {cachenodeid,cacheclusterid,region,aws_account,env}
