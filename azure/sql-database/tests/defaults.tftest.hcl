...
run "as_count_modifier_follows_the_grouping_clause" {
  ...
  assert {
    condition = alltrue([
      !can(regex("\\.as_count\\(\\) by", datadog_monitor.deadlocks[0].query)),
      !can(regex("\\.as_count\\(\\) by", datadog_monitor.connection_failures[0].query)),
    ])
  ...
  assert {
    condition = alltrue([
      can(regex("\\}\\.as_count\\(\\)", datadog_monitor.deadlocks[0].query)),
      can(regex("\\}\\.as_count\\(\\)", datadog_monitor.connection_failures[0].query)),
    ])
