...
run "as_count_modifier_follows_the_grouping_clause" {
  ...
  assert {
    condition = alltrue([
      !can(regex("\\.as_count\\(\\) by", datadog_monitor.pods_failed[0].query)),
      !can(regex("\\.as_count\\(\\) by", datadog_monitor.pods_pending[0].query)),
      !can(regex("\\.as_count\\(\\) by", datadog_monitor.nodes_not_ready[0].query)),
    ])
  ...
  assert {
    condition = alltrue([
      can(regex("\\}\\.as_count\\(\\)", datadog_monitor.pods_failed[0].query)),
      can(regex("\\}\\.as_count\\(\\)", datadog_monitor.pods_pending[0].query)),
      can(regex("\\}\\.as_count\\(\\)", datadog_monitor.nodes_not_ready[0].query)),
    ])
