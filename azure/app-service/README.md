# terraform-datadog-monitor/azure/app-service

Configures monitors for Azure App Service **sites** (`azure.app_services.*`).

For the capacity that sites run on, see `azure/app-service-plan`, which reads a separate metric
namespace (`azure.web_serverfarms.*`). Saturation usually shows up on the plan before it shows up on
an individual site, so most estates want both modules.
...
| Name | Type |
|------|------|
| [datadog_monitor.file_system_usage](https://registry.terraform.io/providers/datadog/datadog/latest/docs/resources/monitor) | resource |
...
| [datadog_monitor.http_4xx_rate](https://registry.terraform.io/providers/datadog/datadog/latest/docs/resources/monitor) | resource |
...
| <a name="input_base_tags"></a> [base\_tags](#input\_base\_tags) | Base tags (key:value format) to add to this type of check (combined with `local.tags` and `var.additional_tags`, generally you should not change this) | `list(string)` | <pre>[<br>  "resource:app-service"<br>]</pre> | no |
...
| <a name="input_file_system_usage_enabled"></a> [file\_system\_usage\_enabled](#input\_file\_system\_usage\_enabled) | Enable App Service filesystem quota monitor (disabled by default; Datadog documents the unit as byte while the description says percentage, so confirm against live data first) | `bool` | `false` | no |
| <a name="input_file_system_usage_evaluation_window"></a> [file\_system\_usage\_evaluation\_window](#input\_file\_system\_usage\_evaluation\_window) | Evaluation window for monitor (`last_?m` (1, 5, 10, 15, or 30), `last_?h` (1, 2, or 4), or `last_1d`] | `string` | `"last_15m"` | no |
| <a name="input_file_system_usage_no_data_window"></a> [file\_system\_usage\_no\_data\_window](#input\_file\_system\_usage\_no\_data\_window) | No data threshold (in minutes, 0 to disable) | `number` | `10` | no |
| <a name="input_file_system_usage_threshold_critical"></a> [file\_system\_usage\_threshold\_critical](#input\_file\_system\_usage\_threshold\_critical) | Filesystem quota consumption at which to alert critical (confirm the metric unit before relying on this) | `number` | `90` | no |
| <a name="input_file_system_usage_threshold_warning"></a> [file\_system\_usage\_threshold\_warning](#input\_file\_system\_usage\_threshold\_warning) | Filesystem quota consumption at which to alert warning (confirm the metric unit before relying on this) | `number` | `80` | no |
| <a name="input_file_system_usage_use_message"></a> [file\_system\_usage\_use\_message](#input\_file\_system\_usage\_use\_message) | Whether to use the query alert base message for App Service filesystem quota monitor | `bool` | `false` | no |
...
| <a name="input_health_check_status_enabled"></a> [health\_check\_status\_enabled](#input\_health\_check\_status\_enabled) | Enable App Service health check monitor (disabled by default; requires a health-check path configured on the app) | `bool` | `false` | no |
...
| <a name="input_health_check_status_use_message"></a> [health\_check\_status\_use\_message](#input\_health\_check\_status\_use\_message) | Whether to use the query alert base message for App Service health check monitor | `bool` | `false` | no |
| <a name="input_http_4xx_rate_enabled"></a> [http\_4xx\_rate\_enabled](#input\_http\_4xx\_rate\_enabled) | Enable App Service HTTP 4xx rate monitor (disabled by default; 4xx on a public app is largely client-driven) | `bool` | `false` | no |
| <a name="input_http_4xx_rate_evaluation_window"></a> [http\_4xx\_rate\_evaluation\_window](#input\_http\_4xx\_rate\_evaluation\_window) | Evaluation window for monitor (`last_?m` (1, 5, 10, 15, or 30), `last_?h` (1, 2, or 4), or `last_1d`] | `string` | `"last_15m"` | no |
| <a name="input_http_4xx_rate_no_data_window"></a> [http\_4xx\_rate\_no\_data\_window](#input\_http\_4xx\_rate\_no\_data\_window) | No data threshold (in minutes, 0 to disable) | `number` | `10` | no |
| <a name="input_http_4xx_rate_threshold_critical"></a> [http\_4xx\_rate\_threshold\_critical](#input\_http\_4xx\_rate\_threshold\_critical) | Percentage of requests returning 4xx at which to alert critical | `number` | `25` | no |
| <a name="input_http_4xx_rate_threshold_warning"></a> [http\_4xx\_rate\_threshold\_warning](#input\_http\_4xx\_rate\_threshold\_warning) | Percentage of requests returning 4xx at which to alert warning | `number` | `10` | no |
| <a name="input_http_4xx_rate_use_message"></a> [http\_4xx\_rate\_use\_message](#input\_http\_4xx\_rate\_use\_message) | Whether to use the query alert base message for App Service HTTP 4xx rate monitor | `bool` | `false` | no |
| <a name="input_http_5xx_rate_enabled"></a> [http\_5xx\_rate\_enabled](#input\_http\_5xx\_rate\_enabled) | Enable App Service HTTP 5xx rate monitor | `bool` | `true` | no |
...
| <a name="input_http_5xx_rate_use_message"></a> [http\_5xx\_rate\_use\_message](#input\_http\_5xx\_rate\_use\_message) | Whether to use the query alert base message for App Service HTTP 5xx rate monitor | `bool` | `false` | no |
...
| <a name="input_response_time_enabled"></a> [response\_time\_enabled](#input\_response\_time\_enabled) | Enable App Service response time monitor | `bool` | `true` | no |
...
| <a name="input_response_time_use_message"></a> [response\_time\_use\_message](#input\_response\_time\_use\_message) | Whether to use the query alert base message for App Service response time monitor | `bool` | `false` | no |
