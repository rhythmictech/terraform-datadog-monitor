...
run "as_count_modifier_follows_the_grouping_clause" {
  ...
  assert {
    condition = alltrue([
      !can(regex("\\.as_count\\(\\) by", datadog_monitor.cold_start_high[0].query)),
      !can(regex("\\.as_count\\(\\) by", datadog_monitor.resiliency_request_timeouts[0].query)),
    ])
  ...
  assert {
    condition = alltrue([
      can(regex("\\}\\.as_count\\(\\)", datadog_monitor.cold_start_high[0].query)),
      can(regex("\\}\\.as_count\\(\\)", datadog_monitor.resiliency_request_timeouts[0].query)),
    ])
